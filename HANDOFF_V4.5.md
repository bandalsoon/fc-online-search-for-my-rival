...
2. 이 문서 `HANDOFF_V4.5.md`
3. `PROJECT_HISTORY_V1_TO_V4.5.md`
4. `V4.5_STABILITY_AUDIT.md`
5. 사용자가 제공한 최신 기능명세서와 모든 첨부 이미지

이 문서는 현재 구현을, 프로젝트 역사 문서는 v1~v4.5의 변경 이유와 폐기된 요구를, 안정성 감사 문서는 아직 수정하지 않은 위험을 담당한다.

## 1. 프로젝트 현재 목적

> 2026-08-31 전술판 후속 작업: 운영 v5를 기준으로 한 v5.1 미리보기에서는 고정 전술판 좌표/X축만 보정을 제약 기반 동적 배치로 대체한다. API·DB·BEST XI 선정·포메이션 표기·통계는 변경하지 않는다. 최신 전술판 규칙은 `DESIGN.md` §13, 검증/승인 상태는 `TACTICAL_V5.1_VALIDATION.md`를 따른다. 이 변경은 화면 승인 전 main에 병합하거나 운영 배포하지 않는다.
...
- 전체 미해결 안정성 항목과 안전한 수정 순서는 `V4.5_STABILITY_AUDIT.md`를 기준으로 한다.
- 공개 `/api/archive`가 임의 닉네임 pair의 비싼 동기화를 만들 수 있으므로 고정 Archive pair 제한이 권장된다.
- D1 load/save/count 오류와 malformed JSON이 정상 빈 데이터처럼 숨겨질 수 있으므로, 불완전 결과를 정상 캐시하지 않는 오류 정책이 필요하다.
- `match_id` 전역 PK와 방향성 pair 조회 간 무결성 위험은 기존 D1 데이터를 조사하기 전 migration하거나 임의 수정하지 않는다.
- 불완전 메타데이터 24시간 캐시, 무제한 Map cache, 급여 실패 반복 재시도, 런타임 응답 검증 부족은 다음 안정화 단계의 대상이다.
- 현재 GitHub build workflow는 포메이션 회귀 테스트를 merge gate로 실행하지 않고 메타데이터 단계도 출력만 하므로 보완이 필요하다.
...
- 구현 기준 이후 `main`에는 인수인계 문서, `AGENTS.md`, Ponytail/Karpathy Codex 스킬이 추가됐다. 이들은 사이트 기능 기준점을 바꾸지 않는다.
