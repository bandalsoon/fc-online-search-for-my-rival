...
function MatchDetail({ match }: { match: Match }) {
  ...
  return <div className="match-detail"><div className="detail-metrics">{metrics.map(([label, a, b, unit]) => <div key={label}><b>{a ?? "-"}{a !== null ? unit : ""}</b><span>{label}</span><b>{b ?? "-"}{b !== null ? unit : ""}</b></div>)}</div>{match.goals.length > 0 && <div className="timeline"><h4>득점 타임라인</h4>{match.goals.map((g, i) => <div className={g.side} key={`${g.minute}:${i}`}><b>{g.minute}'</b><span>{g.scorer}{g.assist ? ` · 도움 ${g.assist}` : ""}</span><em>{g.score}</em></div>)}</div>}<div className="mvp-row">{SIDES.map((s) => match.mvp[s] && <div key={s}><span>실제 평점 최고</span><strong>{match.mvp[s]!.name}</strong><b>{match.mvp[s]!.rating}</b></div>)}</div><div className="lineups"><section><h4>{match.home.nickname} · {match.home.formation}</h4><Pitch players={match.home.lineup} formation={match.home.formation} /></section><section><h4>{match.rival.nickname} · {match.rival.formation}</h4><Pitch players={match.rival.lineup} formation={match.rival.formation} /></section></div></div>;
}
