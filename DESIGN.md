...
### ULTIMATE v5.1 전술판 (2026-08-31 배포 승인)

아래 v5.1 규칙은 이 절의 과거 고정 좌표/X축만 보정 규칙을 대체한다. 포메이션 **표기 구간은 변경하지 않는다**.

- PC 실제 피치 기본 크기 520×760px. 모바일은 실제 가장 좁은 경기 상세 내부 폭과 390px 중 작은 폭, 기본 9:16 비율.
- 같은 환경의 모든 BEST XI/경기 상세에서 공통 폭·높이를 사용한다. 경기 상세의 중첩 패딩까지 반영한 숨겨진 크기 측정 요소로 가용 폭을 구한다.
- BEST XI 카드 간격 24px, 동일 헤더 높이, 형제 카드의 12px 상단 여백 제거. 520px 피치 2개와 패딩·간격이 실제 컨테이너에 들어가는 1156px부터 2열이다.
- 기존 선수 UI 9요소·폰트·순서·공식 이미지·fallback·MOM은 그대로다. 실제 표시 내용이 같은 측정 표본은 중복 제거한다.
- `measureComposite`는 하위 요소와 텍스트의 합집합 및 2px 외곽 여유를 측정한다. 전체 실제 선수 표본의 최대 경계 상자를 모든 전술판 계산에 사용한다.
- 선호 Y는 ST 10 / Wing·CF 23 / AM 36 / CM 50 / DM 64 / 수비 79 / GK 92. 빈 레이어를 지워 등분하지 않는다.
- X는 선호 위치에 가장 가까운 안전 배치를 구한다. 같은 행 전체 UI 폭+간격을 확보하고 피치 가장자리를 지킨다.
- X 투영이 겹치는 레이어 쌍만 전체 높이+안전 간격을 요구한다. 7개 레이어의 상·하한을 계산해 선호 Y를 가능한 범위에 제한한다.
- X가 분리된 경우에도 인접 의미 레이어 사이에 선수 구성 높이의 절반은 확보한다. 1px 차이로 LS/RF가 같은 줄처럼 보이는 실패를 방지하며, 전체 높이+간격 조건은 X가 겹칠 때만 적용한다.
- 3백의 윙백만 DM 레이어로 옮긴다. L/R 반전 금지, GK 중앙·최하단, 동일 레이어 동일 Y를 유지한다.
- 한 행에 공간이 부족하면 11명 모두 동일 배율, 양 팀 중 작은 배율을 공통 적용한다. 안전 간격은 배율 적용 후 PC 12px, 모바일 8px, 360px 미만 6px이다.
- 실제 모든 포메이션에서 필요한 높이의 최댓값을 공통 높이로 적용한다. 폰트/이미지/컨테이너 변경 시 다시 측정하며 스크롤에는 계산하지 않는다.
- 기존 모바일 글자도 작으므로 1 미만 배율은 가독성 확인 대상으로 남긴다. 임의 최소 배율을 적용해 겹침을 숨기지 않는다. 피치 overflow로 선수 정보를 잘라내지 않는다.
- 테스트와 검증 기록: `TACTICAL_V5.1_VALIDATION.md`.

### 이전 v5 좌표 기록 (v5.1에서 대체)
